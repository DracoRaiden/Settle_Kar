...
function LedgerScreen({ group }) {
  const activities = LEDGER_MOCK[group.id] ?? [];
  const netLabel = group.netBalance >= 0 ? "You are owed" : "You owe";
  const netAmount = `Rs. ${Math.abs(group.netBalance)}`;
...
function GroupDetailScreen({ group, onBack }) {
  ...
  return (
    <section className="w-full font-sans">
      ...
      {innerTab === INNER_TABS.ledger && <LedgerScreen group={group} />}
